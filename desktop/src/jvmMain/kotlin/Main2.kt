...
fun mains() {
    ...
    application {
        ...
        val favoritesVM = remember { FavoritesViewModel(scope) }
        val topicViewModel = remember { TopicViewModel(scope) }
        var showThemeSelector by remember { mutableStateOf(false) }
        val themeColors by remember { mutableStateOf(ThemeColors.Default) }
        val isDarkMode = true
        val canCloseOnExit = true
                        ...
                        when (val tab = vm.browserTab.selectedTab()) {
                            is Tabs.PinnedTab<TabType> -> {
                                when ((tab.data as TabType.Pinned).index) {
                                    ...
                                    1 -> AppShow(topicViewModel, favoritesVM)
                                }
                            }
